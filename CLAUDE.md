...
## Поиск научных источников — ЖЕЛЕЗНЫЕ ПРАВИЛА

### ЗАПРЕЩЕНО
- **НИКОГДА не выдумывать источники.** Не генерировать «правдоподобные» названия статей, авторов, журналов.
- Если не можешь найти реальный источник — **честно скажи**: «Я не нашёл подходящего источника по этому запросу через доступные базы».
- Не «округлять» — не выдавать примерное название за точное.

### Как искать источники
1. **WebSearch** — искать по CyberLeninka (cyberleninka.ru), eLibrary.ru, Google Scholar
2. **paper_search (HuggingFace MCP)** — для arXiv (англоязычные STEM)
3. **WebFetch** — для верификации найденного: открыть страницу статьи и подтвердить автора, год, название
4. Каждый найденный источник **должен иметь**: автора, год, точное название, URL или DOI
5. Если пользователь просит «не старше 2023» — фильтровать строго

### Формат выдачи источников
```
Иванов А.А. Название статьи // Журнал. — 2024. — Т. XX, № X. — С. XX-XX.
URL: https://cyberleninka.ru/article/n/...
```

### Российские базы
- **CyberLeninka** (cyberleninka.ru) — бесплатный доступ, полные тексты
- **eLibrary.ru** (РИНЦ) — крупнейшая российская научная база
- **Google Scholar** — с фильтром по языку и году
...
### MCP pdf-mcp (продвинутая работа с PDF)
7 инструментов для инкрементального чтения больших PDF с SQLite FTS5 полнотекстовым поиском.
Для документов >20 страниц — предпочитай pdf-mcp вместо встроенного Read.

### Академические скиллы
- `/deep-research` — 13-агентный pipeline для глубокого исследования (7 режимов: полный, быстрый, обзор литературы, fact-check, сократический диалог, систематический обзор)
- `/academic-paper` — 12-агентный pipeline для написания статей (10 режимов: полный, план, outline, ревизия, abstract, lit-review, конвертация формата)
- `/academic-paper-reviewer` — симуляция рецензирования: 5 рецензентов (EIC + 3 peer + Devil's Advocate)
- `/academic-pipeline` — полный цикл: исследование → написание → рецензирование → ревизия → финализация
...
### source-finder
Поиск и верификация научных источников. Ищет по OpenAlex, CyberLeninka, Semantic Scholar.
Использование: делегируй задачу поиска источников этому агенту.
**Никогда не выдумывает источники** — только реально найденные через базы.

### citation-checker
Проверка цитат и источников в готовом тексте. Верифицирует каждый источник через базы данных.
Выдаёт отчёт: ✓ верифицировано / ✗ не найдено / ⚠ несовпадение.

### document-summarizer
Конспектирование документов с дословными цитатами и номерами страниц.
Использует pdf-mcp для постраничного чтения.

## MCP doc-rag (локальный RAG)
Векторный поиск по проиндексированным документам (ChromaDB).
- `index_directory(path)` — проиндексировать папку с документами
- `search_documents(query)` — семантический поиск по всем документам
- База: `~/.claude/doc-rag-db/`

## MCP cyberleninka (поиск по CyberLeninka)
Прямой поиск по cyberleninka.ru — крупнейшей российской базе открытых научных статей.
- `search_articles(query, year_from, year_to)` — поиск статей
- `get_article(url)` — получить полную информацию о статье
